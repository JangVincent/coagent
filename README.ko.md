...
상태는 `~/.data/agent-chat-cowork/` 아래 보관됩니다:

- `chat.jsonl` — append-only 메시지 로그 (새 클라이언트가 접속하면 마지막 200개를 backlog 로 재전송)
- `sessions/<name>_<cwdhash>.json` — (에이전트, cwd) 별 Claude 세션 ID

다른 위치 쓰고 싶으면: `DATA_DIR=/some/path coagent ...`

전체 초기화는:
```bash
rm -rf ~/.data/agent-chat-cowork
```

또는 채팅 로그만 아카이브하고 새로 시작:
```bash
coagent hub --fresh
```
...
- `DATA_DIR=~/.data/agent-chat-cowork` — 데이터 디렉토리
